...
contract Verifier is IVerifier, Opcode {
    ...
    constructor(address[4] memory _verifiers) {
        ...
        inputsLength[OPCODE_SWAP] = 11;
    }

    function publicInputsLength(uint8 _opcode) public view returns (uint8) {
        ...
        returns (bool)
    {
        ...
        if (_opcode == 4) {
            return IGroth16Verifier(verifiers[4]).verifyProof(_a, _b, _c, publicInputsSwap(_publicInputs));
        }
        return false;
    }
    ...
    function publicInputsSwap(uint256[] calldata _publicInputs) internal pure returns (uint256[11] memory) {
        require(_publicInputs.length == 11);
